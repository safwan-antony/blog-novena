...
from novena.forms import *
from novena_dash.forms import *
from novena_dash.models import *
...
def loginPage(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            if user.is_doctor:
                login(request, user)
                messages.success(request,'Login Successfully doctor')
                return redirect('dash')
            else :
                login(request, user)
                messages.success(request,'Login Successfully as a Patient')
                return redirect('home')
        else:
...
def profilePage(request,pk):
    ...
    appoinment = Appoinment.objects.filter(patient=user)
    education = My_Educational.objects.filter(doctor=request.user)
    context={'user':user,'appoinment':appoinment,'education':education}
...
def editProfilePatient(request,pk):
    ...
    if request.method == 'POST':
        form = ProfilePatientForm(request.POST,request.FILES,instance=patient)
        if form.is_valid():
            form.save()
            return redirect('account_profile',pk=patient.id)
    context = {'form':form,'user':patient}
...
def editProfileDoctor(request,pk):
    ...
    if request.method == 'POST':
        form = ProfileDoctorForm(request.POST,request.FILES,instance=doctor)
        if form.is_valid():
            form.save()
            return redirect('account_profile',pk=doctor.id)
    context = {'form':form,'user':doctor}
    return render(request,'account/edit_profile.html',context)
...
def Change_Password(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Your password was successfully updated! you need to login again with the new password')
            return redirect('home')
